...
ResidualModelAnticipatedStateTpl<Scalar>::ResidualModelAnticipatedStateTpl(boost::shared_ptr<typename Base::StateAbstract> state, const std::size_t nu, 
...
ResidualModelAnticipatedStateTpl<Scalar>::ResidualModelAnticipatedStateTpl(boost::shared_ptr<typename Base::StateAbstract> state,
    ...
    : Base(state, state->get_nv(), true, true, false) {
		time_matrix_ = MatrixXs::Identity(state->get_nv(),state->get_nv()) * anticipated_time;
}
